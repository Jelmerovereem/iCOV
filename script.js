...
const simulation = d3.forceSimulation(nodes.NetworkNodes.Nodes.Node)
  .force("link", d3.forceLink()
      .id(function(d){ return d.id; })
      .links(edges.NetworkEdges.EdgeSets.EdgeSet)
      )
...
const criminalSwitch = document.querySelector(".criminalCheckbox");

function deleteCriminals() {
  console.log(nodes.NetworkNodes.Nodes.Node)
  let withoutCriminals = nodes.NetworkNodes.Nodes.Node.filter(obj => obj.CriminalHistory === false);
  console.log(withoutCriminals)
  const oldNodes = d3.select(".node").selectAll("path").data(withoutCriminals, function(d) {return d.Label});


  /*d3.select("")*/
}

criminalSwitch.addEventListener("change", deleteCriminals);

d3.selectAll(".groupNode")
  .append("text")
    .attr("class", "nodesLabel")
    .attr("transform", function(d) {return `translate(${d.x}, ${d.y})`})
    .attr("fill", function(d, index, paths) {return paths[index].previousSibling.getAttribute("fill")})
    .attr("font-size", "8px")
    .text(function(d) {
      if (d.NodeID === "PEOPLE") {
...
const groupIds = Array.from(
  ...
  .map(function(department) {return department.departmentId;});
...
function dragged(event, d) {
  ...
  link.filter(function(l) { return l.source === event.subject;}).attr("x1", d.x).attr("y1", d.y);
  link.filter(function(l) { return l.target === event.subject;}).attr("x2", d.x).attr("y2", d.y);
}
...
function ticked() {
  link
    ...
    node
        .attr("transform", function(d){return `translate(${d.x - 5}, ${d.y - 7}) scale(${nodeSizeRange.value})`;})

    /*d3.selectAll(".groupNode")
        .attr("transform", function(d){return `translate(${d.x}, ${d.y}) scale(${nodeSizeRange.value})`;})*/

  d3.selectAll(".nodesLabel")
    .attr("transform", function(d) {return `translate(${d.x - 15}, ${d.y + 25})`})

  d3.selectAll(".extrasContainer path")
    .attr("transform", function(d) {return `translate(${d.x - 13}, ${d.y - 14})`})
...
function group_dragged(dragevent, groupId) {
  node
    ...
    .each(function(d) {
      ...
      link.filter(function(l) { return l.source === d;}).attr("x1", d.x += event.movementX).attr("y1", d.y += event.movementY);
      link.filter(function(l) { return l.target === d;}).attr("x2", d.x += event.movementX).attr("y2", d.y += event.movementY);
      //console.log(d.x, d.y)
    })
}
